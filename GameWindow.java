...
public class GameWindow extends JComponent implements KeyListener, MouseListener {
  ...
  // Manages the color and thickness of hitboxes when drawn (if debug mode
  // enabled)
  private static final int BORDER_THICKNESS = 2;
  private static final Color HITBOX_COLOR = Color.MAGENTA;

  // Related to the color, size, and positioning of debug text, drawn when debug
  // enabled.
  ...
  // In order to ensure smooth movement, movement keys remain in the set until
  // they are released
  // Non-movement keys are removed from the set after each call to
  // getKeysPressed()
  private HashSet<Integer> keysCurrentlyPressed;

  // Current game speed, as percentage (ex: 150 = 150% speed, ie 50% faster than
  // normal)
  ...
  // The filenames and Image objects of the currently drawn background/splash
  // images
  ...
  // Tracks the System.nanoTime() of the last completed render, used in the game
  // timing
  ...
  private void initDebugText() {
     ...
     * try{
     * debugFont = Font.createFont(Font.TRUETYPE_FONT, new
     * File(getClass().getResource(DEBUG_FONT_FILE).getFile()));
     * }
     * catch (FontFormatException | IOException e){
     * // System.err.println("Could not open debug font file at: " +
     * DEBUG_FONT_FILE);
     * }
     */
  ...
  public void paintComponent(Graphics g) {
    ...
    // set isPainting to true to ensure logic stops updating until painting is
    // complete
    isPainting = true;
  ...
  // Draws the current background image underneath everything else in the game
  // window
  private void drawBackground(Graphics g) {
    // checks if there's currently a background image set
    // Graphics2D g2D = (Graphics2D) g;
    // RectangularShape backgroundColor = new Rectangle2D.Double(e.getX(), e.getY(),
  ...
  // Updates the debug text displayed in the corner of the game window (if debug
  // mode enabled)
  ...
  // This background color won't be seen if there is a bg image (unless it has
  // transparency)
  ...
  // Non-Movement keys are removed so they aren't re-registered on subsequent
  // checks
  // Movement keys only removed from keysPressed Set when they are released on the
  // keyboard
